//
//  JSON_custom_schema_freddy.swift
...
extension HUBJSONSchemaRegistry{
    var freddySchemaId: String {return "freddyCustomSchema"}
    
    func registerFreddySchema(){
...
//        //go(to: "props").go(to: "treeContent").go(to: "module").go(to: "resource").
        ...
        schema.viewModelSchema.bodyComponentModelDictionariesPath = schema.createNewPath().go(to: "Products").forEach().dictionaryPath()
        schema.componentModelSchema.customDataPath = schema.createNewPath().dictionaryPath()
        
        
        registerCustomSchema(schema, forIdentifier: freddySchemaId)
